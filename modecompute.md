...
```sh
ff-mpirun -np 4 modecompute.md -eps_target 0.1+1.0i -eps_nev 4 -fi <FILEIN> -fo <VECFILE>
```

### Solve eigenvalue problems over a sequence of shifts, store eigenvalue spectra and eigenvectors:
```sh
ff-mpirun -np 4 modecompute.md -eps_target 0.1+0.2i -ntarget 10 -targetf 0.1+2.0i -eps_nev 4 -fi <FILEIN> -fo <VECFILE> -so <VALFILE>
```

NOTE: This file should not be changed unless you know what you're doing.


SEE ALSO: [basecompute.md](./basecompute.md), [basecontinue.md](./basecontinue.md), [basedeflate.md](./basedeflate.md), [foldcompute.md](./foldcompute.md), [hopfcompute.md](./hopfcompute.md)
...
if(fileext == "base") {
  ...
  real beta1, beta2;
  real[int] qm, qma;
  ub[] = loadbota(fileroot, meshin, qm, qma, alpha1, alpha2, beta1, beta2);
}
...
for (int n = 0; n < ntarget; ++n){
  ...
  int k = EPSSolve(J, M, lvectors = lvec, vectors = vec, values = val, IFMACRO(Jsetargs) Jsetargs, ENDIFMACRO
  ...
  for (int ii = 0; ii < val.n; ++ii){
    ChangeNumbering(J, vec[ii][], temp);
    ChangeNumbering(J, vec[ii][], temp, inverse = true);
    if (epstwosided) {
      ChangeNumbering(J, lvec[ii][], temp);
      ChangeNumbering(J, lvec[ii][], temp, inverse = true);
    }
  }
