...
if (mpirank==0){
  ...
  if (fileext == "base"){
    ...
    if (paraviewflag > 1){
      meshN Thgs = trunc(Thg, 1, split = paraviewflag);
      fespace XMhs(Thgs, Pk);
      XMhs defu(us) = defu(ubg), defu(ums) = defu(umg);
      qpv.resize(us[].n);
      qmpv.resize(ums[].n);
      qmapv.resize(ums[].n);
      qpv = us[];
    ...
    ugr[] = qmpv;
    savevtk(workdir + fileout + "_bota_dirmode.vtu", Thgpv, paraviewu(ugr), dataname = ParaViewDataName, order = ParaViewOrder);
    ugr[] = qmapv;
    savevtk(workdir + fileout + "_bota_adjmode.vtu", Thgpv, paraviewu(ugr), dataname = ParaViewDataName, order = ParaViewOrder);
  }
