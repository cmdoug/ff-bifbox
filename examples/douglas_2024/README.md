...
Here:
- $\vec{q}=\left(u_i,p\right)^T \text{ if free outflow BC}$
- $\vec{q}=\left(u_i,p,p_o\right)^T \text{ if modified outflow BC}$
- $\vec{q}=\left(u_i,p,\phi_o\right)^T \text{ if balanced outflow BC}$
