...
```sh
ff-mpirun -np 4 modecomputeaug.md -eps_target 0.1+1.0i -eps_nev 10 -fi <FILEIN> -so <VALFILE>
```

### Solve eigenvalue problem, store eigenvalue spectrum and eigenvectors:
```sh
ff-mpirun -np 4 modecomputeaug.md -eps_target 0.1+1.0i -eps_nev 4 -fi <FILEIN> -fo <VECFILE> -so <VALFILE>
```

### Solve eigenvalue problem, store eigenvectors without storing eigenvalue spectrum:
```sh
ff-mpirun -np 4 modecomputeaug.md -eps_target 0.1+1.0i -eps_nev 4 -fi <FILEIN> -fo <VECFILE>
```

### Solve eigenvalue problems over a sequence of shifts, store eigenvalue spectra and eigenvectors:
```sh
ff-mpirun -np 4 modecomputeaug.md -eps_target 0.1+0.2i -ntarget 10 -targetf 0.1+2.0i -eps_nev 4 -fi <FILEIN> -fo <VECFILE> -so <VALFILE>
```
