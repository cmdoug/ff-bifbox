...
| Wall, $\Gamma_w$ | $u_x=\alpha\hat{n}_y$, $u_y=-\alpha\hat{n}_x$ |
...
This weak formulation has been implemented in the equations file for this example: [eqns_pralits_etal_2010.idp](./eqns_pralits_etal_2010.idp).
...
```sh
FreeFem++-mpi -v 0 examples/pralits_etal_2010/cylinder.md -mo $workdir/cylinder
```
...
```sh
ff-mpirun -np $nproc basecompute.md -v 0 -dir $workdir -mi cylinder.msh -fo cylinder -1/Re 0.1 -alpha 0
```
...
>>>>>>> 5191e4f (Sharpen mesh corner refinement)
```

3. Compute base states at $Re=50$ and $Re=100$ with guesses from continuation
```sh
ff-mpirun -np $nproc basecompute.md -v 0 -dir $workdir -fi cylinder_4.base -fo cylinder50 -1/Re 0.02
ff-mpirun -np $nproc basecompute.md -v 0 -dir $workdir -fi cylinder_6.base -fo cylinder100 -1/Re 0.01
```
...
```sh
cd "$workdir" && set -- cylinder100_*specialpt.base && export B="$1" && export F="$2" && cd -
ff-mpirun -np $nproc foldcompute.md -v 0 -dir $workdir -fi $B -fo cylinder100_B -param alpha -nf 0
ff-mpirun -np $nproc foldcompute.md -v 0 -dir $workdir -fi $F -fo cylinder100_F -param alpha -nf 0
```
...
### Hopf Bifurcations
9. Compute direct eigenmode at $Re=50$, $\alpha=0$
```sh
ff-mpirun -np $nproc modecompute.md -v 0 -dir $workdir -fi cylinder50.base -fo cylindermode1 -eps_target 0.1+0.8i -eps_pos_gen_non_hermitian
```
...
```sh
ff-mpirun -np $nproc hopfcompute.md -v 0 -dir $workdir -fi cylindermode1.mode -fo cylindermode1 -param 1/Re -nf 0
