...
- $`sg\left(r,x\right)=\begin{cases} 1, & \text{if } r \leq r_{sg} \text{ and } x \leq x_{sg} \\\\ 1 + \left(\alpha-1\right)\zeta\left(x,x_{sg}\right), & \text{if } r \leq r_{sg} \text{ and } x > x_{sg} \\\\ sg\left(r_{sg}, x\right) + \left[\alpha-sg\left(r_{sg},x\right)\right]\zeta\left(r,r_{sg}\right), & \text{if } r > r_{sg} \end{cases}`$
