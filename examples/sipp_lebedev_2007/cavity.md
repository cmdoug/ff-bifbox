...
border C07(t=0, 1){x=wcav+(1.75-wcav)*t; y=0; label=BCwall;}
border C08(t=0, 1){x=1.75+(xmax-1.75)*t; y=0; label=BCslip;}
border C09(t=0, 1){x=xmax; y=rmax*t; label=BCopen;}
border C00(t=0, 1){x=xmax+(xmin-xmax)*t; y=rmax; label=BCaxis;}
// Assemble mesh
mesh Thg = buildmesh(C01(n2*rmax) + C02(n1*(0.4-xmin)) + C03(n0*0.4)
                     + C04(n1*dcav) + C05(n2*wcav) + C06(n1*dcav)
                     + C07(n1*(1.75-wcav)) + C08(n1*(xmax-1.75)) + C09(n1*rmax) + C00(n1*(xmax-xmin)));
plot(Thg);
