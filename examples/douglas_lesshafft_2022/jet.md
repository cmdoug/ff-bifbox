...
This file can be used with FreeFEM to create a mesh for confined swirling laminar jets as in [Douglas & Lesshafft. JFM, (2022)](https://doi.org/10.1017/jfm.2022.589).
...
bool confined = getARGV("-confined", 1);
bool flush = getARGV("-flush", 0);

string meshout = getARGV("-mo", "jet_" + (flush ? "flush" : "inject") + "_" + (confined ? "" : "un") + "confined.msh"); // mesh filename
...
if (confined) {
	...
	border C11(t=0, 1){x = 0; y = Rw - (Rw - 0.5 - deltaw)*t; label = flush ? BCwall : BCinternal;}
	border C12(t=0, 1){x = Xinf*t; y = 0.5+deltaw; label = BCinternal;}
	// Assemble mesh
	if (flush)
		Thg = buildmesh(C01(0.5*n0) + C02((Xinf + l)*n1) + C03((0.5 + deltaw)*n2) 
  	    	          + C04((Rw - 0.5 - deltaw)*n2) + C05(Xinf*n2) + C09(deltaw*n0)
										+ C10(l*n0) + C11((Rw - 0.5 - deltaw)*n1) + C12(Xinf*n0));
	else
		Thg = buildmesh(C01(0.5*n0) + C02((Xinf + l)*n1) + C03((0.5 + deltaw)*n2) 
  		              + C04((Rw - 0.5 - deltaw)*n2) + C05(Xinf*n2) + C06(Xw*n1) 
										+ C07((Rw - 0.5 - deltaw)*n1) + C08(Xw*n0) + C09(deltaw*n0)
										+ C10(l*n0) + C11((Rw - 0.5 - deltaw)*n1) + C12(Xinf*n0));
} else {
	// Define borders
	//          o-4-o--..__
	...
	border C09(t=0, 1){x = 0.0; y = Rinf - (Rinf - 0.5 - deltaw)*t; label = flush ? BCwall : BCinternal;}
	// Assemble mesh
	if (flush)
		Thg = buildmesh(C01(0.5*n0) + C02((Rinf + l)*n1) + C03(Rinf*pi/2.0*n2)
										+ C07(deltaw*n0) + C08(l*n0) + C09((Rinf - 0.5 - deltaw)*n1));
	else
		Thg = buildmesh(C01(0.5*n0) + C02((Rinf + l)*n1) + C03(Rinf*pi/2.0*n2)
										+ C04(Xw*n2) + C05((Rinf - 0.5 - deltaw)*n1) + C06(Xw*n0)
										+ C07(deltaw*n0) + C08(l*n0) + C09((Rinf - 0.5 - deltaw)*n1));
}
plot(Thg,wait=1);
